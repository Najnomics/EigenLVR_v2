...
describe("ZkSync Spoke Pool", function () {
    ...
    let hubPool, zkSyncSpokePool, dai, usdc, weth;
    let l2Dai, crossDomainAliasAddress, crossDomainAlias;
    let owner, relayer, rando;
    let zkErc20Bridge, zkUSDCBridge, l2Eth;
    let constructorArgs;
    beforeEach(async function () {
        [owner, relayer, rando] = await utils_1.ethers.getSigners();
        ({ weth, dai, usdc, l2Dai, hubPool } = await (0, HubPool_Fixture_1.hubPoolFixture)());
        ...
        zkErc20Bridge = await smock_1.smock.fake(abiData.erc20DefaultBridge.abi, { address: ERC20_BRIDGE });
        zkUSDCBridge = await smock_1.smock.fake(abiData.erc20DefaultBridge.abi, { address: USDC_BRIDGE });
        ...
        zkSyncSpokePool = await utils_hre_1.hre.upgrades.deployProxy(await (0, utils_1.getContractFactory)("ZkSync_SpokePool", owner), [0, zkErc20Bridge.address, owner.address, hubPool.address], { kind: "uups", unsafeAllow: ["delegatecall"], constructorArgs });
        await (0, utils_1.seedContract)(zkSyncSpokePool, relayer, [dai, usdc], weth, constants_1.amountHeldByPool);
    });
    it("Only cross domain owner upgrade logic contract", async function () {
        // TODO: Could also use upgrades.prepareUpgrade but I'm unclear of differences
        const implementation = await utils_hre_1.hre.upgrades.deployImplementation(await (0, utils_1.getContractFactory)("ZkSync_SpokePool", owner), { kind: "uups", unsafeAllow: ["delegatecall"], constructorArgs });
    ...
    it("Invalid USDC bridge configuration is rejected", async function () {
        ...
        (0, utils_1.expect)(_constructorArgs[1]).to.equal(usdc.address);
        (0, utils_1.expect)(_constructorArgs[2]).to.equal(zkUSDCBridge.address);
        (0, utils_1.expect)(_constructorArgs[3]).to.equal(cctpTokenMessenger);
        // Verify successful deployment.
        let implementation = utils_hre_1.hre.upgrades.deployImplementation(await (0, utils_1.getContractFactory)("ZkSync_SpokePool", owner), {
            kind: "uups",
            unsafeAllow: ["delegatecall"],
            constructorArgs: _constructorArgs,
        });
        await (0, utils_1.expect)(implementation).to.not.be.reverted;
        // Configure cctp
        _constructorArgs = [...constructorArgs];
        _constructorArgs[2] = ZERO_ADDRESS;
        _constructorArgs[3] = (0, utils_1.randomAddress)();
        implementation = utils_hre_1.hre.upgrades.deployImplementation(await (0, utils_1.getContractFactory)("ZkSync_SpokePool", owner), {
            kind: "uups",
            unsafeAllow: ["delegatecall"],
            constructorArgs: _constructorArgs,
        });
        ...
        await (0, utils_1.expect)(implementation).to.be.reverted;
    ...
    it("Bridge tokens to hub pool correctly calls the Standard L2 Bridge for zkSync Bridged USDC.e", async function () {
        ...
        const implementation = await utils_hre_1.hre.upgrades.deployImplementation(await (0, utils_1.getContractFactory)("ZkSync_SpokePool", owner), { kind: "uups", unsafeAllow: ["delegatecall"], constructorArgs });
        await zkSyncSpokePool.connect(crossDomainAlias).upgradeTo(implementation);
        const { leaves, tree } = await (0, MerkleLib_utils_1.constructSingleRelayerRefundTree)(usdc.address, await zkSyncSpokePool.callStatic.chainId());
        await zkSyncSpokePool.connect(crossDomainAlias).relayRootBundle(tree.getHexRoot(), constants_1.mockTreeRoot);
        await zkSyncSpokePool.connect(relayer).executeRelayerRefundLeaf(0, leaves[0], tree.getHexProof(leaves[0]));
        // This should have sent tokens back to L1. Check the correct methods on the gateway are correctly called.
        (0, utils_1.expect)(zkErc20Bridge.withdraw).to.have.been.calledOnce;
        (0, utils_1.expect)(zkErc20Bridge.withdraw).to.have.been.calledWith(hubPool.address, usdc.address, constants_1.amountToReturn);
    });
    it("Bridge tokens to hub pool correctly calls the custom USDC L2 Bridge for Circle Bridged USDC", async function () {
        ...
        (0, utils_1.expect)(allowance.isZero()).to.be.true;
        await zkSyncSpokePool.connect(relayer).executeRelayerRefundLeaf(0, leaves[0], tree.getHexProof(leaves[0]));
        ...
        (0, utils_1.expect)(allowance.eq(constants_1.amountToReturn)).to.be.true;
        (0, utils_1.expect)(zkUSDCBridge.withdraw).to.have.been.calledOnce;
        (0, utils_1.expect)(zkUSDCBridge.withdraw).to.have.been.calledWith(hubPool.address, usdc.address, constants_1.amountToReturn);
    });
