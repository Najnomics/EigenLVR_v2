...
describe("svm_spoke.bundle", () => {
    ...
    const nonOwner = web3_js_1.Keypair.generate();
    const relayerA = web3_js_1.Keypair.generate();
    const relayerB = web3_js_1.Keypair.generate();
    let state, seed, mint, relayerTA, relayerTB, vault, transferLiability;
    ...
    before(async () => {
        ...
        relayerTA = (await (0, spl_token_1.getOrCreateAssociatedTokenAccount)(connection, payer, mint, relayerA.publicKey)).address;
        relayerTB = (await (0, spl_token_1.getOrCreateAssociatedTokenAccount)(connection, payer, mint, relayerB.publicKey)).address;
        ...
        chai_1.assert.strictEqual(BigInt(initialVaultBalance), BigInt(initialMintAmount), "Initial vault balance should be equal to the minted amount");
        [transferLiability] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from("transfer_liability"), mint.toBuffer()], program.programId);
    });
    it("Relays Root Bundle", async () => {
        ...
        const [rootBundle] = web3_js_1.PublicKey.findProgramAddressSync(seeds, program.programId);
        ...
        try {
            await program.methods
                ...
                .signers([nonOwner])
                .rpc();
            chai_1.assert.fail("Non-owner should not be able to relay root bundle");
        }
        catch (err) {
            chai_1.assert.include(err.toString(), "Only the owner can call this function!", "Expected owner check error");
        }
        ...
        chai_1.assert.isTrue(relayerRefundRootHex === relayerRefundRootBuffer.toString("hex"), "Relayer refund root should be set");
        chai_1.assert.isTrue(slowRelayRootHex === slowRelayRootBuffer.toString("hex"), "Slow relay root should be set");
        // Check that the root bundle index has been incremented
        stateAccountData = await program.account.state.fetch(state);
        chai_1.assert.isTrue(stateAccountData.rootBundleId.toString() === "1", "Root bundle index should be 1");
        ...
        const [rootBundle2] = web3_js_1.PublicKey.findProgramAddressSync(seeds2, program.programId);
        ...
        await program.methods
            ...
            .rpc();
        stateAccountData = await program.account.state.fetch(state);
        chai_1.assert.isTrue(stateAccountData.rootBundleId.toString() === "2", "Root bundle index should be 2");
    });
    ...
    it("Simple Leaf Refunds Relayers", async () => {
        ...
        const relayerARefund = new anchor_1.BN(400000);
        const relayerBRefund = new anchor_1.BN(100000);
        relayerRefundLeaves.push({
            isSolana: true,
            leafId: new anchor_1.BN(0),
            chainId: chainId,
            amountToReturn: new anchor_1.BN(69420),
            mintPublicKey: mint,
            refundAddresses: [relayerA.publicKey, relayerB.publicKey],
            refundAmounts: [relayerARefund, relayerBRefund],
        });
        const merkleTree = new utils_1.MerkleTree(relayerRefundLeaves, web3_v1_1.relayerRefundHashFn);
        ...
        const instructionParams = await (0, web3_v1_1.loadExecuteRelayerRefundLeafParams)(program, owner, stateAccountData.rootBundleId, leaf, proofAsNumbers);
        const tx = await program.methods
        ...
        chai_1.assert.isNull(instructionParamsInfo, "Instruction params account should be closed");
        ...
        chai_1.assert.strictEqual(BigInt(iVaultBal) - BigInt(fVaultBal), BigInt(totalRefund), "Vault balance");
        chai_1.assert.strictEqual(BigInt(fRelayerABal) - BigInt(iRelayerABal), BigInt(relayerARefund.toString()), "Relayer A bal");
        chai_1.assert.strictEqual(BigInt(fRelayerBBal) - BigInt(iRelayerBBal), BigInt(relayerBRefund.toString()), "Relayer B bal");
        ...
        try {
            ...
            await (0, web3_v1_1.loadExecuteRelayerRefundLeafParams)(program, owner, stateAccountData.rootBundleId, leaf, proofAsNumbers);
            await program.methods
                .executeRelayerRefundLeaf()
                .accounts(executeRelayerRefundLeafAccounts)
                .remainingAccounts(remainingAccounts)
                .rpc();
            chai_1.assert.fail("Leaf should not be executed multiple times");
        }
        catch (err) {
            chai_1.assert.include(err.toString(), "Leaf already claimed!", "Expected claimed leaf error");
        }
    });
    it("Test Merkle Proof Verification", async () => {
        ...
        const invalidRelayerRefundLeaf = {
            isSolana: true,
            leafId: new anchor_1.BN(solanaDistributions + 1),
            chainId: chainId,
        ...
        const leaf = relayerRefundLeaves[solanaLeafNumber];
        const proofAsNumbers = proof.map((p) => Array.from(p));
        let stateAccountData = await program.account.state.fetch(state);
        const rootBundleId = stateAccountData.rootBundleId;
        const rootBundleIdBuffer = Buffer.alloc(4);
        ...
        try {
            ...
            await program.methods
                ...
                .rpc();
            chai_1.assert.fail("Should not execute to invalid refund address");
        }
        catch (err) {
            chai_1.assert.include(err.toString(), "Invalid refund address");
        }
        // Verify valid leaf
        executeRelayerRefundLeafAccounts = {
            state: state,
        ...
        chai_1.assert.strictEqual(BigInt(iVaultBal) - BigInt(fVaultBal), BigInt(totalRefund), "Vault balance");
        chai_1.assert.strictEqual(BigInt(fRelayerABal) - BigInt(iRelayerABal), BigInt(leaf.refundAmounts[0].toString()), "Relayer A bal");
        chai_1.assert.strictEqual(BigInt(fRelayerBBal) - BigInt(iRelayerBBal), BigInt(leaf.refundAmounts[1].toString()), "Relayer B bal");
        // Verify invalid leaf
        try {
            ...
            await (0, web3_v1_1.loadExecuteRelayerRefundLeafParams)(program, owner, stateAccountData.rootBundleId, invalidRelayerRefundLeaf, proofAsNumbers);
            await program.methods
                .executeRelayerRefundLeaf()
                .accounts(executeRelayerRefundLeafAccounts)
                .remainingAccounts(remainingAccounts)
                .rpc();
            chai_1.assert.fail("Invalid leaf should not be verified");
        }
        catch (err) {
            chai_1.assert.include(err.toString(), "Invalid Merkle proof");
        }
    });
    it("Test Merkle Proof Verification with Mixed Solana and EVM Leaves", async () => {
        ...
        for (let i = 0; i < relayerRefundLeaves.length; i += 1) {
            // Only Solana leaves
            if (!relayerRefundLeaves[i].isSolana)
                continue;
            const leaf = relayerRefundLeaves[i];
        ...
        const totalRefund = relayerRefundLeaves
            ...
            .reduce((acc, leaf) => acc.add(leaf.refundAmounts[0].add(leaf.refundAmounts[1])), new anchor_1.BN(0))
            .toString();
        chai_1.assert.strictEqual(BigInt(iVaultBal) - BigInt(fVaultBal), BigInt(totalRefund), "Vault balance");
        chai_1.assert.strictEqual(BigInt(fRelayerABal) - BigInt(iRelayerABal), BigInt(relayerRefundLeaves
            .filter((leaf) => leaf.isSolana)
            .reduce((acc, leaf) => acc.add(leaf.refundAmounts[0]), new anchor_1.BN(0))
            .toString()), "Relayer A bal");
        chai_1.assert.strictEqual(BigInt(fRelayerBBal) - BigInt(iRelayerBBal), BigInt(relayerRefundLeaves
            .filter((leaf) => leaf.isSolana)
            .reduce((acc, leaf) => acc.add(leaf.refundAmounts[1]), new anchor_1.BN(0))
            .toString()), "Relayer B bal");
    });
    ...
    it("Execute Leaf Refunds Relayers with invalid chain id", async () => {
        ...
        try {
            ...
            chai_1.assert.include(err.toString(), "Invalid chain id");
        }
    });
    it("Execute Leaf Refunds Relayers with invalid mintPublicKey", async () => {
        ...
        relayerRefundLeaves.push({
            ...
            chainId: chainId,
            amountToReturn: new anchor_1.BN(0),
            mintPublicKey: web3_js_1.Keypair.generate().publicKey,
            refundAddresses: [relayerA.publicKey, relayerB.publicKey],
            refundAmounts: [relayerARefund, relayerBRefund],
        });
        const merkleTree = new utils_1.MerkleTree(relayerRefundLeaves, web3_v1_1.relayerRefundHashFn);
        ...
        try {
            ...
            chai_1.assert.include(err.toString(), "Invalid mint");
        }
    });
    it("Sequential Leaf Refunds Relayers", async () => {
        const relayerRefundLeaves = [];
        const relayerRefundAmount = new anchor_1.BN(100000);
        // Generate 5 sequential leaves
        for (let i = 0; i < 5; i++) {
            relayerRefundLeaves.push({
                isSolana: true,
                leafId: new anchor_1.BN(i),
                chainId: chainId,
                amountToReturn: new anchor_1.BN(0),
                mintPublicKey: mint,
        ...
        for (let i = 0; i < 5; i++) {
            ...
            await (0, web3_v1_1.loadExecuteRelayerRefundLeafParams)(program, owner, stateAccountData.rootBundleId, relayerRefundLeaves[i], proof[i]);
            await program.methods
                .executeRelayerRefundLeaf()
                .accounts(executeRelayerRefundLeafAccounts)
                .remainingAccounts(remainingAccounts)
        ...
        const fVaultBal = (await connection.getTokenAccountBalance(vault)).value.amount;
        const fRelayerABal = (await connection.getTokenAccountBalance(relayerTA)).value.amount;
        const totalRefund = relayerRefundAmount.mul(new anchor_1.BN(5)).toString();
        chai_1.assert.strictEqual(BigInt(iVaultBal) - BigInt(fVaultBal), BigInt(totalRefund), "Vault balance");
        chai_1.assert.strictEqual(BigInt(fRelayerABal) - BigInt(iRelayerABal), BigInt(totalRefund), "Relayer A bal");
    ...
    it("Should allow the owner to delete the root bundle", async () => {
        ...
        try {
            ...
            await program.methods
                ...
                .signers([nonOwner])
                .rpc();
            chai_1.assert.fail("Non-owner should not be able to delete the root bundle");
        }
        catch (err) {
            chai_1.assert.include(err.toString(), "NotOwner", "Expected error for non-owner trying to delete root bundle");
        }
        ...
        try {
            ...
            chai_1.assert.include(err.toString(), "Account does not exist", "Expected error when fetching deleted root bundle");
        }
        ...
        const [newRootBundle] = web3_js_1.PublicKey.findProgramAddressSync(newSeeds, program.programId);
        chai_1.assert.isTrue(stateAccountData.rootBundleId === initialRootBundleId + 1, `Root bundle index should be ${initialRootBundleId + 1}`);
        ...
        stateAccountData = await program.account.state.fetch(state);
        chai_1.assert.isTrue(stateAccountData.rootBundleId === initialRootBundleId + 2, `Root bundle index should be ${initialRootBundleId + 2}`);
        chai_1.assert.isTrue(newRelayerRefundRootHex === newRelayerRefundRootBuffer.toString("hex"), "New relayer refund root should be set");
        chai_1.assert.isTrue(newSlowRelayRootHex === newSlowRelayRootBuffer.toString("hex"), "New slow relay root should be set");
    });
    describe("Execute Max Refunds", () => {
        const executeMaxRefunds = async (testConfig) => {
            chai_1.assert.isTrue(!(testConfig.deferredRefunds && testConfig.atomicAccountCreation), "Incompatible test configuration");
            ...
            const refundAccounts = []; // These would hold either token accounts or claim accounts.
            const refundAddresses = []; // These are relayer authority addresses used in leaf building.
            const refundAmounts = [];
            for (let i = 0; i < testConfig.solanaDistributions; i++) {
                // Will create token account later if needed.
                const tokenOwner = web3_js_1.Keypair.generate().publicKey;
                const tokenAccount = (0, spl_token_1.getAssociatedTokenAddressSync)(mint, tokenOwner);
                refundAddresses.push(tokenOwner);
                const [claimAccount] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from("claim_account"), mint.toBuffer(), tokenOwner.toBuffer()], program.programId);
                if (!testConfig.deferredRefunds && !testConfig.atomicAccountCreation) {
                    await (0, spl_token_1.getOrCreateAssociatedTokenAccount)(connection, payer, mint, tokenOwner);
            ...
            // Verify valid leaf
            const proofAsNumbers = proof.map((p) => Array.from(p));
            const [instructionParams] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from("instruction_params"), owner.toBuffer()], program.programId);
            ...
            // Execute using ALT.
            await (0, web3_v1_1.sendTransactionWithLookupTable)(connection, instructions, anchor.AnchorProvider.env().wallet.payer);
    ...
    it("Increments pending amount to HubPool", async () => {
        ...
        const incrementPendingToHubPool = async (amountToReturn) => {
            const relayerRefundLeaves = [];
            relayerRefundLeaves.push({
                isSolana: true,
                leafId: new anchor_1.BN(0),
        ...
        chai_1.assert.isTrue(pendingToHubPool.eq(initialPendingToHubPool), "Pending amount should not have changed");
        const firstAmountToReturn = new anchor_1.BN(1_000_000);
        ...
        chai_1.assert.isTrue(pendingToHubPool.eq(initialPendingToHubPool.add(firstAmountToReturn)), "Pending amount should be incremented by first amount");
        const secondAmountToReturn = new anchor_1.BN(2_000_000);
        await incrementPendingToHubPool(secondAmountToReturn);
        pendingToHubPool = (await program.account.transferLiability.fetch(transferLiability)).pendingToHubPool;
        chai_1.assert.isTrue(pendingToHubPool.eq(initialPendingToHubPool.add(firstAmountToReturn.add(secondAmountToReturn))), "Pending amount should be incremented by second amount");
    });
    it("Reversed Relayer Leaf Refunds", async () => {
        ...
        const fVaultBal = (await connection.getTokenAccountBalance(vault)).value.amount;
        const fRelayerABal = (await connection.getTokenAccountBalance(relayerTA)).value.amount;
        const totalRefund = relayerRefundAmount.mul(new anchor_1.BN(numberOfRefunds)).toString();
        chai_1.assert.strictEqual(BigInt(iVaultBal) - BigInt(fVaultBal), BigInt(totalRefund), "Vault balance");
        chai_1.assert.strictEqual(BigInt(fRelayerABal) - BigInt(iRelayerABal), BigInt(totalRefund), "Relayer A bal");
    });
    it("Invalid Merkle Leaf should fail", async () => {
        ...
        try {
            ...
            chai_1.assert.instanceOf(err, anchor.AnchorError);
    ...
    describe("Deferred refunds in ExecutedRelayerRefundRoot events", () => {
        const executeRelayerRefundLeaf = async (testConfig) => {
            ...
            if (!testConfig.deferredRefunds) {
                refundA = (await (0, spl_token_1.getOrCreateAssociatedTokenAccount)(connection, payer, mint, relayerA.publicKey)).address;
                refundB = (await (0, spl_token_1.getOrCreateAssociatedTokenAccount)(connection, payer, mint, relayerB.publicKey)).address;
            }
            else {
                [refundA] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from("claim_account"), mint.toBuffer(), relayerA.publicKey.toBuffer()], program.programId);
                [refundB] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from("claim_account"), mint.toBuffer(), relayerB.publicKey.toBuffer()], program.programId);
    ...
    it("Cannot execute relayer refund leaf with insufficient pool balance", async () => {
        ...
        const relayerRefundLeaves = [];
        const relayerARefund = new anchor_1.BN(vaultBal).add(new anchor_1.BN(1));
        relayerRefundLeaves.push({
            isSolana: true,
            leafId: new anchor_1.BN(0),
            chainId: chainId,
        ...
        try {
            await program.methods
                ...
                .remainingAccounts(remainingAccounts)
                .rpc();
            chai_1.assert.fail("Leaf execution should fail due to insufficient pool balance");
        }
        catch (err) {
            chai_1.assert.instanceOf(err, anchor.AnchorError);
            chai_1.assert.strictEqual(err.error.errorCode.code, "InsufficientSpokePoolBalanceToExecuteLeaf", "Expected error code InsufficientSpokePoolBalanceToExecuteLeaf");
        }
    });
    it("Fails Leaf Verification Without Leading 64 Bytes", async () => {
        ...
        const customRelayerRefundHashFn = (input) => {
            input = input;
            const refundAmountsBuffer = Buffer.concat(input.refundAmounts.map((amount) => {
    ...
    describe("Execute Max multiple refunds with claims", async () => {
        const executeMaxRefundClaims = async (testConfig) => {
            ...
            for (let i = 0; i < testConfig.solanaDistributions; i++) {
                ...
                const tokenOwner = web3_js_1.Keypair.generate().publicKey;
                const tokenAccount = (await (0, spl_token_1.getOrCreateAssociatedTokenAccount)(connection, payer, mint, tokenOwner)).address;
                refundAddresses.push(tokenOwner);
                tokenAccounts.push(tokenAccount);
                const [claimAccount] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from("claim_account"), mint.toBuffer(), tokenOwner.toBuffer()], program.programId);
                // Create instruction to initialize claim account.
                initializeInstructions.push(await program.methods.initializeClaimAccount().accounts({ mint, refundAddress: tokenOwner }).instruction());
                ...
                claimInstructions.push(await program.methods.claimRelayerRefund().accounts(claimRelayerRefundAccounts).instruction());
            }
            const { relayerRefundLeaves, merkleTree } = (0, utils_2.buildRelayerRefundMerkleTree)({
                totalEvmDistributions: evmDistributions,
                totalSolanaDistributions: testConfig.solanaDistributions,
            ...
            if (!testConfig.separatePhases) {
                ...
                if (testConfig.useAddressLookup)
                    ...
                    await anchor_1.web3.sendAndConfirmTransaction(connection, new anchor_1.web3.Transaction().add(...instructions), [anchor.AnchorProvider.env().wallet.payer], {
                ...
                if (testConfig.useAddressLookup) {
                    await (0, web3_v1_1.sendTransactionWithLookupTable)(connection, initializeInstructions, anchor.AnchorProvider.env().wallet.payer);
                    await (0, web3_v1_1.sendTransactionWithLookupTable)(connection, [executeInstruction], anchor.AnchorProvider.env().wallet.payer);
                    await (0, web3_v1_1.sendTransactionWithLookupTable)(connection, claimInstructions, anchor.AnchorProvider.env().wallet.payer);
                }
                else {
                    await anchor_1.web3.sendAndConfirmTransaction(connection, new anchor_1.web3.Transaction().add(...initializeInstructions), [anchor.AnchorProvider.env().wallet.payer], {
                        commitment: "confirmed",
                    });
                    await anchor_1.web3.sendAndConfirmTransaction(connection, new anchor_1.web3.Transaction().add(executeInstruction), [anchor.AnchorProvider.env().wallet.payer], {
                        commitment: "confirmed",
                    });
                    await anchor_1.web3.sendAndConfirmTransaction(connection, new anchor_1.web3.Transaction().add(...claimInstructions), [anchor.AnchorProvider.env().wallet.payer], {
                        commitment: "confirmed",
                    });
                }
            }
            // Verify all refund token account balances.
            const refundBalances = await Promise.all(tokenAccounts.map(async (account) => {
                return (await connection.getTokenAccountBalance(account)).value.amount;
            }));
            refundBalances.forEach((balance, i) => {
                assertSE(balance, refundAmounts[i].toString(), `Refund account ${i} balance should match refund amount`);
