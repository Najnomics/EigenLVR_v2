...
describe("svm_spoke.slow_fill", () => {
    ...
    let state, seed, mint, relayerTA, recipientTA, otherRelayerTA, vault, fillStatus;
    ...
    const relaySlowFillRootBundle = async (slowRelayLeafRecipient = recipient, slowRelayLeafChainId = chainId, message = encodedMessage) => {
        ...
        const slowRelayLeaf = {
            ...
            updatedOutputAmount: new anchor_1.BN(relayAmount),
        };
        await updateRelayData(slowRelayLeaf.relayData);
        slowRelayLeafs.push(slowRelayLeaf);
        const merkleTree = new MerkleTree_1.MerkleTree(slowRelayLeafs, web3_v1_1.slowFillHashFn);
        ...
        const relayHash = (0, web3_v1_1.calculateRelayHashUint8Array)(slowRelayLeaf.relayData, chainId);
    ...
    before("Creates token mint and associated token accounts", async () => {
        mint = await (0, spl_token_1.createMint)(connection, payer, owner, owner, tokenDecimals);
        relayerTA = (await (0, spl_token_1.getOrCreateAssociatedTokenAccount)(connection, payer, mint, relayer.publicKey)).address;
        otherRelayerTA = (await (0, spl_token_1.getOrCreateAssociatedTokenAccount)(connection, payer, mint, otherRelayer.publicKey)).address;
        await (0, spl_token_1.mintTo)(connection, payer, mint, relayerTA, owner, seedBalance);
        await (0, spl_token_1.mintTo)(connection, payer, mint, otherRelayerTA, owner, seedBalance);
    ...
    beforeEach(async () => {
        ...
        vault = (await (0, spl_token_1.getOrCreateAssociatedTokenAccount)(connection, payer, mint, state, true)).address; // Initialize vault
        // mint mint to vault
        ...
        assert.strictEqual(BigInt(initialVaultBalance), BigInt(initialMintAmount), "Initial vault balance should be equal to the minted amount");
    ...
    it("Executes slow relay leaf, verify the event & state change", async () => {
        ...
        assertSE(event.relayExecutionInfo.updatedMessageHash, (0, web3_v1_1.hashNonEmptyMessage)(relayData.message), "UpdatedMessageHash should match");
