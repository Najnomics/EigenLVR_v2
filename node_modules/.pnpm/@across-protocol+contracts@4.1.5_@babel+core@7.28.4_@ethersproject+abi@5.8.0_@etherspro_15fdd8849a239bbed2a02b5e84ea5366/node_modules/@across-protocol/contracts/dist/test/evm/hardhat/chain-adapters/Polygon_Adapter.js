...
const IOFT__factory_1 = require("../../../../typechain/factories/contracts/interfaces/IOFT__factory");
const consts_1 = require("../../../../deploy/consts");
const typechain_1 = require("../../../../typechain");
const constants_2 = require("@across-protocol/constants");
let hubPool, polygonAdapter, weth, dai, usdc, usdt, matic, timer, mockSpoke;
let l2Weth, l2Dai, l2WMatic, l2Usdc, l2Usdt;
let owner, dataWorker, liquidityProvider;
let rootChainManager, fxStateSender, depositManager, cctpMessenger, cctpTokenMinter, erc20Predicate, oftMessenger, adapterStore;
const polygonChainId = constants_2.CHAIN_IDs.POLYGON;
const oftArbitrumEid = (0, utils_1.getOftEid)(polygonChainId);
describe("Polygon Chain Adapter", function () {
    beforeEach(async function () {
        [owner, dataWorker, liquidityProvider] = await utils_1.ethers.getSigners();
        ({ weth, dai, l2Weth, l2Dai, hubPool, mockSpoke, timer, usdc, l2Usdc, usdt, l2Usdt } = await (0, HubPool_Fixture_1.hubPoolFixture)());
        ...
        await (0, utils_1.seedWallet)(dataWorker, [dai, matic, usdc, usdt], weth, constants_1.amountToLp);
        await (0, utils_1.seedWallet)(liquidityProvider, [dai, matic, usdc, usdt], weth, constants_1.amountToLp.mul(10));
        await (0, HubPool_Fixture_1.enableTokensForLP)(owner, hubPool, weth, [weth, dai, matic, usdc, usdt]);
        ...
        oftMessenger = await (0, utils_1.createTypedFakeFromABI)([...IOFT__factory_1.IOFT__factory.abi]);
        adapterStore = await (0, utils_1.createTypedFakeFromABI)([...typechain_1.AdapterStore__factory.abi]);
        const oftFeeCap = (0, utils_1.toWei)("1");
        polygonAdapter = await (await (0, utils_1.getContractFactory)("Polygon_Adapter", owner)).deploy(rootChainManager.address, fxStateSender.address, depositManager.address, erc20Predicate, matic.address, weth.address, usdc.address, cctpMessenger.address, adapterStore.address, oftArbitrumEid, oftFeeCap);
    ...
    it("Correctly calls the OFT bridge adapter when attempting to bridge USDT", async function () {
        ...
        const oftMessengerType = utils_1.ethers.utils.formatBytes32String("OFT_MESSENGER");
        await adapterStore
            .connect(owner)
        ...
        const { leaves, tree, tokensSendToL2 } = await (0, MerkleLib_utils_1.constructSingleChainTree)(usdt.address, 1, internalChainId, 6);
        await hubPool
            .connect(dataWorker)
            .proposeRootBundle([3117], 1, tree.getHexRoot(), constants_1.mockRelayerRefundRoot, constants_1.mockSlowRelayRoot);
        await timer.setCurrentTime(Number(await timer.getCurrentTime()) + constants_1.refundProposalLiveness + 1);
        // Mock OFT fees and send receipts
        const msgFeeStruct = [(0, utils_1.toWei)("0"), utils_1.ethers.BigNumber.from(0)];
        oftMessenger.quoteSend.returns(msgFeeStruct);
        const msgReceipt = [
            utils_1.ethers.utils.hexlify(utils_1.ethers.utils.randomBytes(32)),
            utils_1.ethers.BigNumber.from("1"),
            msgFeeStruct,
        ];
        const oftReceipt = [tokensSendToL2, tokensSendToL2];
        oftMessenger.send.returns([msgReceipt, oftReceipt]);
        await hubPool.connect(dataWorker).executeRootBundle(...Object.values(leaves[0]), tree.getHexProof(leaves[0]));
        // Adapter should have approved OFT messenger to spend USDT from HubPool
        (0, utils_1.expect)(await usdt.allowance(hubPool.address, oftMessenger.address)).to.equal(tokensSendToL2);
        const sendParam = {
            dstEid: oftArbitrumEid,
            to: utils_1.ethers.utils.hexZeroPad(mockSpoke.address, 32).toLowerCase(),
        ...
        (0, utils_1.expect)(oftMessenger.send).to.have.been.calledOnce;
        (0, utils_1.expect)(oftMessenger.send).to.have.been.calledWith(sendParam, msgFeeStruct, hubPool.address);
    });
});
