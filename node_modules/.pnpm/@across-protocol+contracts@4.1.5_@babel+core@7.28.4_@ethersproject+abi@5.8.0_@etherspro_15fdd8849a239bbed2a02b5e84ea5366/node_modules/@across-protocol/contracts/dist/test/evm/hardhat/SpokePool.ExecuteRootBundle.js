...
describe("SpokePool Root Bundle Execution", function () {
    ...
    it("Execute relayer root correctly with mixed solana and evm leaves", async function () {
        ...
        const { relayerRefundLeaves: leaves, merkleTree: tree } = (0, utils_2.buildRelayerRefundMerkleTree)({
    ...
    it("Execution correctly logs deferred refunds", async function () {
        ...
        (0, utils_1.expect)(await destErc20.balanceOf(spokePool.address)).to.equal(consts.amountHeldByPool.sub(consts.amountToRelay));
        ...
        let relayTokensEvents = await spokePool.queryFilter(spokePool.filters.ExecutedRelayerRefundRoot());
        (0, utils_1.expect)(relayTokensEvents[0].args?.deferredRefunds).to.equal(true);
    });
    describe("_distributeRelayerRefunds", function () {
        ...
        describe("amountToReturn > 0", function () {
            ...
            it("emits TokensBridged", async function () {
                await (0, utils_1.expect)(spokePool
                    ...
                    .withArgs((0, utils_1.toBN)(1), destinationChainId, 0, (0, utils_1.addressToBytes)(destErc20.address), dataWorker.address);
            });
        });
        describe("amountToReturn = 0", function () {
        ...
        describe("Total refundAmounts > spokePool's balance", function () {
            it("Reverts and emits log", async function () {
                await (0, utils_1.expect)(spokePool.connect(dataWorker).distributeRelayerRefunds(destinationChainId, (0, utils_1.toBN)(1), [consts.amountHeldByPool, consts.amountToRelay], // spoke has only amountHeldByPool.
                0, destErc20.address, [relayer.address, rando.address])).to.be.revertedWith("InsufficientSpokePoolBalanceToExecuteLeaf");
                (0, utils_1.expect)((await destErc20.queryFilter(destErc20.filters.Transfer(spokePool.address))).length).to.equal(0);
            });
        });
    });
});
