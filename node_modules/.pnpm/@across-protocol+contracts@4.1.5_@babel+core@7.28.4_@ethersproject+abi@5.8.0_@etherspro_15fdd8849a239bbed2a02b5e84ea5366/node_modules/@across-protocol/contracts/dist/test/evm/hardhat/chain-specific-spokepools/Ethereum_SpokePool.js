...
describe("Ethereum Spoke Pool", function () {
    ...
    it("Only owner can set the hub pool address", async function () {
        await (0, utils_1.expect)(spokePool.connect(rando).setWithdrawalRecipient(rando.address)).to.be.reverted;
        await spokePool.connect(owner).setWithdrawalRecipient(rando.address);
        (0, utils_1.expect)(await spokePool.withdrawalRecipient()).to.equal(rando.address);
    });
