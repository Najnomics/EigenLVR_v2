...
exports.Signer = exports.keccak256 = exports.defaultAbiCoder = exports.BigNumber = exports.ethers = exports.Contract = exports.expect = exports.isBytes32 = exports.bytes32ToAddress = exports.addressToBytes = exports.hexZeroPad = exports.createRandomBytes32 = exports.hexToUtf8 = exports.utf8ToHex = exports.toBN = exports.fromWei = exports.toBNWeiWithDecimals = exports.toBNWei = exports.toWeiWithDecimals = exports.toWei = exports.SignerWithAddress = void 0;
...
exports.createTypedFakeFromABI = createTypedFakeFromABI;
exports.trimSolanaAddress = trimSolanaAddress;
exports.hashNonEmptyMessage = hashNonEmptyMessage;
exports.avmL1ToL2Alias = avmL1ToL2Alias;
exports.getOftEid = getOftEid;
...
const hexZeroPad = (input, length) => hardhat_1.ethers.utils.hexZeroPad(input, length);
exports.hexZeroPad = hexZeroPad;
const addressToBytes = (input) => (0, exports.hexZeroPad)(input.toLowerCase(), 32);
exports.addressToBytes = addressToBytes;
const bytes32ToAddress = (input) => {
    if (!/^0x[a-fA-F0-9]{64}$/.test(input)) {
        throw new Error("Invalid bytes32 input");
    }
    return hardhat_1.ethers.utils.getAddress("0x" + input.slice(26));
};
exports.bytes32ToAddress = bytes32ToAddress;
const isBytes32 = (input) => /^0x[0-9a-fA-F]{64}$/.test(input);
...
function randomBytes32() {
    return hardhat_1.ethers.utils.hexlify(hardhat_1.ethers.utils.randomBytes(32));
}
...
async function createTypedFakeFromABI(abi, targetAddress = "") {
...
function trimSolanaAddress(bytes32Address) {
    if (!hardhat_1.ethers.utils.isHexString(bytes32Address, 32)) {
        throw new Error("Invalid bytes32 address");
    }
    const uint160Address = hardhat_1.ethers.BigNumber.from(bytes32Address).mask(160);
    return hardhat_1.ethers.utils.hexZeroPad(hardhat_1.ethers.utils.hexlify(uint160Address), 20);
}
function hashNonEmptyMessage(message) {
    if (!hardhat_1.ethers.utils.isHexString(message) || message.length % 2 !== 0)
        throw new Error("Invalid hex message bytes");
    // account for 0x prefix when checking length
    if (message.length > 2) {
        return hardhat_1.ethers.utils.keccak256(message);
    }
    return hardhat_1.ethers.utils.hexlify(new Uint8Array(32));
}
const { defaultAbiCoder, keccak256 } = hardhat_1.ethers.utils;
exports.defaultAbiCoder = defaultAbiCoder;
exports.keccak256 = keccak256;
function getOftEid(chainId) {
    const value = consts_1.OFT_EIDs.get(chainId);
    if (value === undefined) {
