...
async function main() {
    ...
    const config = {
        ...
        amountToDepositIntoSpokePool: (0, utils_1.toBN)("200000000000000000"),
    };
    console.log(`Script config: `, config);
    // Initialize contracts:
    const SpokePoolArtifact = (0, utils_1.findArtifactFromPath)("ZkSync_SpokePool", `${__dirname}/../artifacts-zk/contracts`);
    const SpokePool = new ethers_1.ContractFactory(SpokePoolArtifact.abi, SpokePoolArtifact.bytecode, signer);
    const spokePool = await SpokePool.attach(config.spokePoolAddress);
    const ERC20 = await (0, utils_1.getContractFactory)("ERC20", { signer });
    const erc20 = await ERC20.attach(config.l2Erc20AddressToTest);
    const WETH = new ethers_1.Contract(config.wethAddress, exports.weth9Abi, signer);
    ...
    (0, console_1.assert)(wethAddress === weth.address, "SpokePool.wrappedNativeToken !== wethAddress");
    ...
    if (config.amountToDepositIntoSpokePool.gt(0) &&
        ...
        console.log(`Approving SpokePool to spend ${config.amountToDepositIntoSpokePool} of ${config.l2Erc20AddressToTest}`, await txn.wait());
    }
