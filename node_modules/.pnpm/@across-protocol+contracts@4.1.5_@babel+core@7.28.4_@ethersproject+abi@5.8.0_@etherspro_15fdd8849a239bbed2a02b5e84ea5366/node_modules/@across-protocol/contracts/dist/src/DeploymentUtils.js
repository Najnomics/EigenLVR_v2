...
exports.getAllDeployedAddresses = getAllDeployedAddresses;
...
exports.DEPLOYMENTS = deployments_;
/**
 * Returns the deployed address of any contract on any network.
 */
function getDeployedAddress(contractName, networkId, throwOnError = true) {
    const address = exports.DEPLOYMENTS[networkId.toString()]?.[contractName]?.address;
 ...
 */
function getAllDeployedAddresses(contractName) {
    const results = [];
    Object.keys(exports.DEPLOYMENTS).forEach((_chainId) => {
        const info = exports.DEPLOYMENTS[_chainId]?.[contractName];
        if (info?.address) {
...
function getDeployedBlockNumber(contractName, networkId) {
    try {
        return exports.DEPLOYMENTS[networkId.toString()][contractName].blockNumber;
    }
    catch (_) {
        throw new Error(`Contract ${contractName} not found on ${networkId} in deployments.json`);
    }
...
function getContractInfoFromAddress(contractAddress) {
    const returnValue = [];
    Object.keys(exports.DEPLOYMENTS).forEach((_chainId) => Object.keys(exports.DEPLOYMENTS[_chainId]).forEach((_contractName) => {
        if (exports.DEPLOYMENTS[_chainId][_contractName].address === contractAddress)
