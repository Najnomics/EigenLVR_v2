...
(0, config_1.task)("enableToken", "Enable a provided token across the entire ecosystem of supported chains")
    ...
    .addOptionalParam("customoptimismbridge", "Custom token bridge to set for optimism, for example used with SNX and DAI")
    ...
    const l1TokenAddr = constants_1.TOKEN_SYMBOLS_MAP[matchedSymbol].addresses[hubChainId];
    (0, assert_1.default)(l1TokenAddr !== undefined, `Could not find ${symbol} in TOKEN_SYMBOLS_MAP`);
    ...
    const tokens = Object.fromEntries(chainIds.map((chainId) => {
        ...
        return [chainId, { symbol: symbol, address }];
    }));
    console.table(Object.entries(tokens).map(([_chainId, { symbol, address }]) => ({ chainId: Number(_chainId), symbol, address })), ["chainId", "symbol", "address"]);
    // Check the user is ok with the token addresses provided. If not, abort.
    if (!(await (0, utils_1.askYesNoQuestion)("\nDo these token addresses match your expectations?")))
        process.exit(0);
    ...
    if (lpTokenAddr === ZERO_ADDRESS) {
        ...
        const doBurn = await (0, utils_1.askYesNoQuestion)(`\nBurn ${burn} ${symbol} (${depositAmount}) LP tokens? (RECOMMENDED!)`);
        if (doBurn) {
            callData[l1Token.address] ??= [];
            callData[l1Token.address].push(l1Token.interface.encodeFunctionData("approve", [hubPool.address, depositAmount]));
        }
        // Create LP token and seed the LP with `depositAmount` amount.
        callData[hubPool.address] ??= [];
        callData[hubPool.address].push(hubPool.interface.encodeFunctionData("enableL1TokenForLiquidityProvision", [l1TokenAddr]));
        callData[hubPool.address].push(hubPool.interface.encodeFunctionData("addLiquidity", [l1Token.address, depositAmount]));
        ...
        if (doBurn) {
            ...
            callData[lpTokenAddr].push(erc20.attach(lpTokenAddr).interface.encodeFunctionData("transfer", [burnRecipient, depositAmount]));
        }
    }
    ...
    routeChainIds.forEach((fromId) => {
        ...
        routeChainIds.forEach((toId) => {
            ...
            if (depositRouteChains.length === 0 ||
                ...
                callData[hubPool.address] ??= [];
                callData[hubPool.address].push(hubPool.interface.encodeFunctionData("setDepositRoute", [fromId, toId, inputToken, enableRoute]));
            }
            else {
    ...
    chainIds.forEach((toId) => {
        ...
        if (depositRouteChains.length === 0 || depositRouteChains.includes(toId)) {
            const n = (++i).toString().padStart(2, " ");
            console.log(`\t${n} Setting rebalance route for chain ${symbol} ${hubChainId} -> ${destinationToken} on ${toId}.`);
            callData[hubPool.address] ??= [];
            callData[hubPool.address].push(hubPool.interface.encodeFunctionData("setPoolRebalanceRoute", [toId, l1TokenAddr, destinationToken]));
        }
        else {
    ...
    if (depositRouteChains.includes(ARBITRUM)) {
        ...
        const spokePool = new ethers.Contract(hubPoolDeployment.address, utils_1.minimalSpokePoolInterface, signer);
        ...
        callData[hubPool.address] ??= [];
        callData[hubPool.address].push(hubPool.interface.encodeFunctionData("relaySpokePoolAdminFunction", [ARBITRUM, whitelistTokenCallData]));
    }
    ...
    if (depositRouteChains.includes(OPTIMISM) && taskArguments.customoptimismbridge) {
        ...
        callData[hubPool.address].push(hubPool.interface.encodeFunctionData("relaySpokePoolAdminFunction", [OPTIMISM, setTokenBridgeCallData]));
    }
