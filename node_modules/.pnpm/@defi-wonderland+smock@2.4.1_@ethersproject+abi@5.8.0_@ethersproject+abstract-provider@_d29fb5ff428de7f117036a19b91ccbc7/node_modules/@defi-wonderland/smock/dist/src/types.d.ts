...
type Abi = ReadonlyArray<Fragment | Pick<JsonFragment, 'name' | 'type' | 'anonymous' | 'payable' | 'constant' | 'stateMutability' | 'inputs' | 'outputs'> | string>;
export type FakeContractSpec = {
    abi?: Abi;
    interface?: Interface;
...
export type SmockContractBase<T extends BaseContract> = Omit<BaseContract, 'connect'> & Omit<T, 'connect'> & {
    wallet: Signer;
...
export type CallOverrideCallback = (address: Buffer, data: Buffer) => Promise<{
    result: Buffer;
