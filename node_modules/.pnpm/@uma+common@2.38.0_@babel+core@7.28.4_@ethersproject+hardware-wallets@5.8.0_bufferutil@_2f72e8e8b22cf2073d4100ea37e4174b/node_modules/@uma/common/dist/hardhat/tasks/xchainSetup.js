...
async function setupOvmBasedL1Chain(hre_, chainId) {
    ...
    const ChildMessenger = await companionNetworks[chainName.toLowerCase()].deployments.get(`${chainName}_ChildMessenger`);
...
(0, config_1.task)("setup-l1-base-xchain", "Configures L1 cross chain smart contracts for Base bridge").setAction(async function (_, hre_) {
...
(0, config_1.task)("setup-l1-blast-xchain", "Configures L1 cross chain smart contracts for Blast bridge").setAction(async function (_, hre_) {
...
(0, config_1.task)("setup-l2-admin-xchain", "Configures L2 cross chain smart contracts with admin messenger").setAction(async function (_, hre_) {
    const hre = hre_;
...
(0, config_1.task)("verify-xchain", "Checks ownership state of cross chain smart contracts")
    ...
    const [governorHub, oracleHub, governor, parentMessenger, childMessenger, oracleSpoke, governorSpoke, l1Registry, l2Registry, l2Store, l2IdentifierWhitelist, l2AddressWhitelist, l2OptimisticOracle, l2OptimisticOracleV2, l2OptimisticOracleV3, l2Finder,] = await Promise.all([
    ...
    const [optimisticOracleRegistered, optimisticOracleV2Registered, optimisticOracleV3Registered, l2RegistryOwner,] = await Promise.all([
    ...
    const l2OptimisticOracleV3Contract = new l2Web3.eth.Contract(l2OptimisticOracleV3.abi, l2OptimisticOracleV3.address);
    ...
    const [l2FinderOwner, l2FinderStore, l2FinderIdentifierWhitelist, l2FinderAddressWhitelist, l2FinderOracle, l2FinderOptimisticOracle, l2FinderOptimisticOracleV2, l2FinderOptimisticOracleV3, l2FinderChildMessenger,] = await Promise.all([
        ...
        l2FinderContract.methods.interfacesImplemented(utf8ToHex(Constants_1.interfaceName.OptimisticOracleV2)).call(),
        l2FinderContract.methods.interfacesImplemented(utf8ToHex(Constants_1.interfaceName.OptimisticOracleV3)).call(),
    ...
    console.log(`- Set "${Constants_1.interfaceName.OptimisticOracleV2}" in Finder: ${l2FinderOptimisticOracleV2 === l2OptimisticOracleV2.address ? "✅" : "❌"}`);
    console.log(`- Set "${Constants_1.interfaceName.OptimisticOracleV3}" in Finder: ${l2FinderOptimisticOracleV3 === l2OptimisticOracleV3.address ? "✅" : "❌"}`);
...
(0, config_1.task)("verify-admin-xchain", "Checks state of L2 smart contracts with admin messenger").setAction(async function (_, hre_) {
    const hre = hre_;
    const { deployments, web3 } = hre;
    const [childMessenger, oracleSpoke, governorSpoke, l2Registry, l2Store, l2IdentifierWhitelist, l2AddressWhitelist, l2OptimisticOracle, l2OptimisticOracleV2, l2OptimisticOracleV3, l2Finder,] = await Promise.all([
