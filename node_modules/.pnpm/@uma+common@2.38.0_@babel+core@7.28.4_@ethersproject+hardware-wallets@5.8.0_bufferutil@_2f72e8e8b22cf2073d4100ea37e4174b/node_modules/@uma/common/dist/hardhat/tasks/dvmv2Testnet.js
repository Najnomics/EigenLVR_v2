...
const { padRight, utf8ToHex } = web3_1.default.utils;
...
async function getContractsForFinder(hre_, mockOracle) {
    const { deployments, web3 } = hre_;
    const supportedFinderContracts = Object.keys(Constants_1.interfaceName);
    const contractsForFinder = new Map();
    for (const contractName of supportedFinderContracts) {
        ...
        if (mockOracle && contractName === "VotingV2")
            continue;
        if (!mockOracle && contractName === "MockOracleAncillary")
            continue;
        try {
...
async function getContractsForRegistry(hre_) {
    const { deployments } = hre_;
    const contractsForRegistry = new Map();
...
(0, config_1.task)("setup-dvmv2-testnet", "Configures DVMv2 on L1 testnet")
    ...
    for (const [contractName, contract] of contractsForFinder) {
        const nameInFinder = Constants_1.interfaceName[contractName];
        ...
        if (currentlySetAddress !== contract.options.address) {
            const txn = await finder.methods
                ...
                .send({ from: deployer });
            console.log(`Set ${contractName} in Finder to "${nameInFinder}" @ ${contract.options.address}, tx: ${txn.transactionHash}`);
        }
        else {
    ...
    const hasMinterRole = await votingToken.methods.holdsRole(Enums_1.TokenRolesEnum.MINTER, VotingV2.address).call();
    if (!hasMinterRole) {
        const txn = await votingToken.methods.addMember(Enums_1.TokenRolesEnum.MINTER, VotingV2.address).send({ from: deployer });
    ...
    const hasProposerRole = await governorV2.methods.holdsRole(Enums_1.GovernorV2RolesEnum.PROPOSER, ProposerV2.address).call();
    if (!hasProposerRole) {
        const txn = await governorV2.methods
            .resetMember(Enums_1.GovernorV2RolesEnum.PROPOSER, ProposerV2.address)
            .send({ from: deployer });
    ...
    const hasEmergencyProposerRole = await governorV2.methods
        .holdsRole(Enums_1.GovernorV2RolesEnum.EMERGENCY_PROPOSER, EmergencyProposer.address)
        .call();
    if (!hasEmergencyProposerRole) {
        const txn = await governorV2.methods
            .resetMember(Enums_1.GovernorV2RolesEnum.EMERGENCY_PROPOSER, EmergencyProposer.address)
            .send({ from: deployer });
        console.log(`Reset emergency proposer role to EmergencyProposer @ ${EmergencyProposer.address}, tx: ${txn.transactionHash}`);
    }
    else {
