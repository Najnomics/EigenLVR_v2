...
const LZ_WRAPPED_TOKEN_BRIDGES = {
...
(0, config_1.task)("whitelist-collateral", "Approve single currency to collateral whitelist and set its final fee")
    .addParam("address", "Collateral address to whitelist", "", config_1.types.string)
    .addParam("finalfee", "Raw amount of final fee to set in Store", "0", config_1.types.string)
...
async function _findL2TokenFromTokenList(l1Web3, l2chainid, l1TokenAddress) {
    ...
    if (l2chainid == 8453) {
        ...
        const body = await response.text();
        const tokenList = JSON.parse(body).tokens;
        const searchSymbol = tokenList.find((element) => element.chainId == 1 && element.address.toLowerCase() == l1TokenAddress.toLowerCase())?.symbol;
        if (!searchSymbol)
            return index_1.ZERO_ADDRESS;
        const found = tokenList.find((element) => element.chainId == 8453 && element.symbol == searchSymbol);
        return found?.address ?? index_1.ZERO_ADDRESS;
    }
    if (l2chainid == 42161) {
...
async function _findL2TokenFromLZBridge(l1Web3, l2Web3, l2chainid, l1TokenAddress) {
    const l1chainid = await l1Web3.eth.getChainId();
    const layerZeroOriginChainId = LZ_CHAIN_IDS[l1chainid];
    (0, assert_1.default)(layerZeroOriginChainId !== undefined, `Missing LayerZero mapping for chain ${l1chainid}`);
    const wrappedTokenBridgeAddress = LZ_WRAPPED_TOKEN_BRIDGES[l2chainid];
    (0, assert_1.default)(wrappedTokenBridgeAddress !== undefined, `Missing WrappedTokenBridge mapping for chain ${l2chainid}`);
